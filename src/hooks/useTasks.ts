...
export const useTasks = () => {
  ...
  const loadStoredFilters = async () => {
    try {
      const storedFilters = await AsyncStorage.getItem(StorageKeysEnum.FILTERS_STORAGE_KEY);
      if (storedFilters) {
  ...
  const saveFilters = async (newFilters: FilterParams) => {
    try {
      await AsyncStorage.setItem(StorageKeysEnum.FILTERS_STORAGE_KEY, JSON.stringify(newFilters));
    } catch (error) {
